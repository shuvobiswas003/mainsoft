...
?>
<form action="bulk_slip_stu_single_data.php" method="POST" target="_blank">
    ...
    <div class="form-group">
        <label class="col-md-3 control-label" for="Cust-name">Select STU ID</label>
        <div class="col-md-9">
...
<div class="row">
    ...
    <INPUT type="button" value="Remove" onclick="deleteRow('multiSlip')" class="btn btn-danger"/>
                ...
                <tbody>
                    <?php
                    $count=1;
                    $sel_query="SELECT * FROM studentpayment WHERE stuid='$uniqid' AND status='Unpaid' ORDER BY id";
                    $result = mysqli_query($link,$sel_query);
                    while($row = mysqli_fetch_assoc($result)) {
                    ?>
                    <tr>
 ...
 <td>
    <input type="date" name="invoice_date[]" class="form-control" required="1" value="<?php echo date('Y-m-d'); ?>" required="1">
                        ...
                        <td>
                        <select name="desss[]" id="" class="form-control" >
                            <option value="<?php echo $row['des'];?>"><?php echo $row['des'];?></option>
                        </select>
                        </td>

                        <td>
                        <select name="total[]" id="" class="form-control" style="width: 100px;">
                            <option value="<?php echo $row['total'];?>"><?php echo $row['total'];?></option>
                        </select>
                        </td>

                        <td>
                        <select name="Prev_pay[]" id="" class="form-control">
                            <option value="<?php echo $row['totalpay'];?>"><?php echo $row['totalpay'];?></option>
                        </select>
                        </td>

                        <td>
                            <input type="number" name="paid_amount[]" class="form-control" required="1"  autofocus='autofocus' max="<?php echo $row['due']?>">
                        </td>
    ...
    <input type="submit" value="Recive Payment" class="btn btn-primary">
</form>                              
    <?php } ?>
    </div>
 <!--2nd Form Part Start-->
