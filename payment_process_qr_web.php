...
// Check if qrResult is set and extract stuid from it
if (isset($_POST['qrResult'])) {
    $qrResult = $_POST['qrResult'];

    // Parse the URL to extract the query string
    $parsedUrl = parse_url($qrResult);

    // Initialize stuid variable
    $stuid = '';

    // Check if the query component is present
    if (isset($parsedUrl['query'])) {
        // Parse the query string into an associative array
        parse_str($parsedUrl['query'], $queryParams);

        // Check if stuid is present in the query parameters
        if (isset($queryParams['stuid'])) {
            $stuid = $queryParams['stuid'];
        }
    }

    // Output or use the extracted stuid
    if (!$stuid) {
        die("Student ID not found in QR result.");
    }
} else {
    die("QR Result not set.");
}
