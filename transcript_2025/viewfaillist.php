...
<table class="table table-bordered">
	<thead>
		<tr>
			...
			<th colspan='2'>Failed Subject's</th>
	...
	<tbody>
        ...
        require "../interdb.php";
        $count2=1;
        $sel_query2="Select * from meritlist where examid='$examid' AND classnumber='$classnumber' AND status='Fail' ORDER BY curpos ASC";
        $result2 = mysqli_query($link,$sel_query2);
        while($row2 = mysqli_fetch_assoc($result2)) {
        ?>
        <tr>
...
while ($row10 = mysqli_fetch_assoc($result10)) {
    ...
    if ($subcode === '101' || $subcode === '102') {
        $banglaTotalMark += $mark;
    }elseif ($subcode === '107' || $subcode === '108') {
        $englishTotalMark += $mark;
...
if ($banglaTotalMark < 50) {
   $failedSubjects[] = [
                'subject' => 'Bangla',
                'mark' => $mark
            ];
}
if ($englishTotalMark < 50) {
   $failedSubjects[] = [
                'subject' => 'English',
                'mark' => $mark
            ];
