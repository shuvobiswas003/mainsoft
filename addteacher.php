...
<?php
 ...
 if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        require "interdb.php";
        $sql = "INSERT INTO teacher (name, tfname, tmname, tdob, leatestdegree, mob, degnation, teachersl, joindate) 
    ...
    if(mysqli_query($link, $sql)){
        echo "<h3 style='color:green;'>Teacher Added Successfully</h1>.";
    } else{
       echo "<h3 style='color:red;'>Teacher Already Exists</h1>". mysqli_error($link);
    }
    mysqli_close($link);
...
<div class="col-md-6">
    <div class="form-group">
        ...
        <div class="col-md-9">
            <input type="text" id="Cust-name" name="name" class="form-control" placeholder="Teacher's Name" required="1">
        </div>
    </div>
...
<div class="col-md-6">
    <div class="form-group">
        <label class="col-md-3 control-label" for="Cust-name">Teacher Sl Number</label>
        <div class="col-md-9">
            <input type="number" id="Cust-name" name="teachersl" class="form-control" placeholder="Teacher Sl Number"  required="1">
        </div>
    </div>
    <div class="form-group">
                    ...
                    <?php
                        ...
                        while($row = mysqli_fetch_assoc($result)) {?>
                            <tr>
                                ...
                                <td>
                                    <a href="view_teacher.php?id=<?php echo $row['id'];?>" target="_blank"><button type="button" class="btn btn-primary" >Profile</button> </a>
                                    <a href="teacheredit.php?id=<?php echo $row['id'];?>"><button type="button" class="btn btn-primary" >Edit</button> </a>
                                    <a href="deleteteacher.php?id=<?php echo $row['id'];?>"><button type="button" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this Teacher?');">Delete</button> </a>
                                </td>
                            </tr>
                    <?php $count++; } ?>
            </tbody>
