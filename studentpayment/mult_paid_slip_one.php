...
<body>
<?php
// Check if serialized_data is posted
if (isset($_POST['serialized_data'])) {
    $serialized_data_json = $_POST['serialized_data'];
    $serialized_data = json_decode($serialized_data_json, true); // Convert JSON string to array
    // echo "<pre>";
    // var_dump($serialized_data);
    // echo "<pre>";
    // Access the arrays
    $payid = $serialized_data['all_puniids'];
    $suniqid = $serialized_data['suniqid'];
...
$idn =$iid[$i];
$count=1;
$sel_query="Select * from studentpayment where puniid='$idn';";
