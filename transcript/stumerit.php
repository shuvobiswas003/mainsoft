...
<section class="transcript">
	<?php
        ...
        $sql = "DELETE FROM meritlist WHERE classnumber='$classnumber' AND examid='$examid'";

if (mysqli_query($link, $sql)) {
    echo "<h3 style='color:green;'>Previous Merit position Deleted</h3>";
    echo "<br>";
} else {
    echo "<h3 style='color:red;'>Failed to delete previous merit position</h3>";
}
...
?>
	...
	require '../interdb.php';
	$count=1;
	$sel_query="SELECT * FROM meritlist WHERE examid='$examid' AND classnumber='$classnumber' ORDER BY failsub ASC,gpa DESC,obtainmark DESC,prevpos ASC;";
	$result = mysqli_query($link,$sel_query);
	while($row = mysqli_fetch_assoc($result)) {
		$id=$row['id'];
		$curpos=$count;
