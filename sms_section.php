                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            ...
                            <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                                ...
                                <div class="form-group">
                                    <label for="secgroup">Select Section Group:</label>
                                ...
                                <div class="form-group">
                                    <label for="start_roll">Start Roll Number:</label>
                                    <input type="number" class="form-control" id="start_roll" name="start_roll" required>
                                </div>

                                <div class="form-group">
                                    <label for="end_roll">End Roll Number:</label>
                                    <input type="number" class="form-control" id="end_roll" name="end_roll" required>
                                </div>
                            ...
                            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                                ...
                                $startRoll = $_POST['start_roll'];
                                $endRoll = $_POST['end_roll'];
                                $message_raw = $_POST['message'];

                                // Prepare SQL statement to select students within the roll number range
                                $sql = "SELECT * FROM student WHERE classnumber = ? AND secgroup = ? AND roll BETWEEN ? AND ?";
                                $stmt = $link->prepare($sql);
                                if (!$stmt) {
                                    die("Error preparing query: " . mysqli_error($link));
                                }
                                
                                $stmt->bind_param("ssss", $class, $secgroup, $startRoll, $endRoll);
                                if (!$stmt->execute()) {
                                ...
                                while ($student = $studentsResult->fetch_assoc()) {
                                    $message = "প্রিয় " . $student['name'] . " " . $message_raw;
                                    $studentRecipientNumber = $student['mobile'];
                                    echo $message;
                                    echo "<br>";
                                   
                                    if (sendSMS($studentRecipientNumber, $message)) {
                                        echo "<p style='color:green;'>SMS sent to {$student['name']} Roll: {$student['roll']}</p>";
                                    } else {
