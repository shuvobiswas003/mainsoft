...
//insert to sturegsubject
    ...
    //class nine/ten spacial pass fail
    if($classnumber=='10'){
        if($subcode=='154'){
            require "interdb.php";
        ...
        if($practicaln<=0){
            $excq=($subfullmarks*70)/100;
            $exmcq=($subfullmarks*30)/100;
            $gcqpass=round($excq/3);
            $gmcqpass=round($exmcq/3);
            echo "Cq Pass".$gcqpass;
            echo "<br> MCq Pass".$gmcqpass;
            
            if($cqn>=$gcqpass AND $mcqn>=$gmcqpass){
                require "interdb.php";
                //fatch data from grade
                $sel_query="SELECT * FROM grademark WHERE gradecode='$gradecode' AND ($total BETWEEN markfrom AND markupto)";
                $result = mysqli_query($link,$sel_query);
                ...
                //endining with no pracical mark
            }else{

            //practical mark codeing start;
            $excq=($subfullmarks*50)/100;
            $exmcq=($subfullmarks*25)/100;
            $gcqpass=round($excq/3);
            $gmcqpass=round($exmcq/3);
            echo "Cq Pass".$gcqpass;
            echo "<br> MCq Pass <br>".$gmcqpass;
    ...
    echo "CQ Mark: ".$cqn."<br>";
    echo "CQ 70% Mark: ".$mcqn."<br>";
    echo "CA Mark: ".$practicaln."<br>";
    echo "Total: ".$total."<br>";
