...
// Fetch daily attendance report by class and section with cinoutid = 1
$dailyReport = [];
foreach ($classesSections as $cs) {
    $sqlDaily = "SELECT 
                    COUNT(CASE WHEN cinout = 'IN' THEN 1 END) AS present,
                    COUNT(CASE WHEN cinout != 'IN' THEN 1 END) AS absent
                 FROM stuattdancedata 
                 WHERE adate = CURDATE() AND cinoutid = 1 AND stuid IN (
                     SELECT stuid FROM student 
                     WHERE classnumber = '{$cs['classnumber']}' AND secgroup = '{$cs['secgroup']}'
                 )";
    $resultDaily = $link->query($sqlDaily);
    $dailyReport["{$cs['classnumber']}-{$cs['secgroup']}"] = $resultDaily->fetch_assoc();
}

// Fetch monthly attendance report by class and section with cinoutid = 1
$monthlyReport = [];
foreach ($classesSections as $cs) {
    $sqlMonthly = "SELECT 
                    COUNT(CASE WHEN cinout = 'IN' THEN 1 END) AS present,
                    COUNT(CASE WHEN cinout != 'IN' THEN 1 END) AS absent
                 FROM stuattdancedata 
                 WHERE MONTH(adate) = MONTH(CURDATE()) AND YEAR(adate) = YEAR(CURDATE()) 
                 AND cinoutid = 1 AND stuid IN (
                     SELECT stuid FROM student 
                     WHERE classnumber = '{$cs['classnumber']}' AND secgroup = '{$cs['secgroup']}'
                 )";
    $resultMonthly = $link->query($sqlMonthly);
    $monthlyReport["{$cs['classnumber']}-{$cs['secgroup']}"] = $resultMonthly->fetch_assoc();
}

// Fetch daily present percentage report by class and section with cinoutid = 1
$percentageReport = [];
foreach ($classesSections as $cs) {
    $sqlPercentage = "SELECT 
                        (COUNT(CASE WHEN cinout = 'IN' THEN 1 END) / COUNT(*)) * 100 AS present_percentage
                      FROM stuattdancedata 
                      WHERE adate = CURDATE() AND cinoutid = 1 
                      AND stuid IN (
                          SELECT stuid FROM student 
                          WHERE classnumber = '{$cs['classnumber']}' AND secgroup = '{$cs['secgroup']}'
                      )";
                ...
                <div class="content">
                    <div class="container">
                        ...
                        <marquee behavior="slow" direction="">
                            <h1 style="color:green;">Welcome <?php echo $_SESSION["teachername"];?></h1>
                        </marquee>
