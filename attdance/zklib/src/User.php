...
class User
{
     ...
     * @return bool|mixed
     */
    public function set(ZKLib $self, $uid, $userid, $name, $password, $role = Util::LEVEL_USER, $cardno)
{
    ...
    if (
        ...
        strlen($password) > 8 ||
        $cardno < 0 ||
        $cardno > 4294967295 // Maximum value of a 32-bit unsigned integer
    ) {
        return false;
    }
    ...
    $byte2 = chr((int)($uid >> 8));
    $cardno = pack('V', (int)$cardno); // Convert decimal card number to binary (4-byte unsigned integer)
