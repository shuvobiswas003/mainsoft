...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['assign_teacher'])) {
        ...
        foreach ($periods_to_assign as $period_data) {
            ...
            // Check if teacher is already assigned to another class in this time slot
            $teacherConflictCheck = "SELECT cr.id, sg.classname, sg.secgroupname, p.period_name
                                   FROM class_routine cr
                                   JOIN sectiongroup sg ON cr.section_id = sg.id
                                   JOIN class_periods p ON cr.period_id = p.id
                                   WHERE cr.day_of_week = ? 
                                   AND cr.period_id = ? 
                                   AND cr.teacher_id = ?";
            $stmt = mysqli_prepare($link, $teacherConflictCheck);
            mysqli_stmt_bind_param($stmt, 'iii', $day, $period_id, $teacher_id);
            mysqli_stmt_execute($stmt);
            $teacherConflictResult = mysqli_stmt_get_result($stmt);
            
            if (mysqli_num_rows($teacherConflictResult) > 0) {
                $conflict = mysqli_fetch_assoc($teacherConflictResult);
                $_SESSION['error'] = "Teacher conflict! This teacher is already assigned to {$conflict['classname']}-{$conflict['secgroupname']} during {$conflict['period_name']}";
                header("Location: class_routine.php?section_id=$section_id");
                exit;
            }
                ...
                <?php else: ?>
                    <?php foreach ($sections as $section): ?>
                        <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                            <a href="class_routine.php?section_id=<?= $section['id'] ?>" class="card section-card text-decoration-none">
        ...
        <div id="routinePanel" class="<?= $currentSectionId ? '' : 'd-none' ?>">
            <div class="fixed-header no-print">
                <div class="d-flex justify-content-between align-items-center">
                    <a href="class_routine.php" class="btn btn-outline-secondary">
