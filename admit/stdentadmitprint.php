...
function renderTable($data) {
    echo "<table border='0' id='subtab'>";
    echo "<tr><th>Sub. Code</th><th>Sub. Name</th><th>Exam Date</th><th>Exam Time</th></tr>";
    foreach ($data as $row15) {
        echo "<tr>";
        echo "<td>{$row15['subcode']}</td>";
        echo "<td>{$row15['subname']}</td>";
        echo "<td>{$row15['examdate']}</td>";
        echo "<td>{$row15['examtime']}</td>";
        echo "</tr>";
    }
    echo "</table>";
}
...
// Fetch subjects the student is registered for
$subjects = [];
$sel_query11 = "SELECT subcode FROM sturegsubject WHERE uniqid='$uniqid' AND (substatus=1 OR substatus=4)";
$result11 = mysqli_query($link, $sel_query11);
while ($row11 = mysqli_fetch_assoc($result11)) {
    $subjects[] = $row11['subcode'];
}

// Fetch and sort exam schedule by date
$exam_data = [];
if (!empty($subjects)) {
    $subcodes = "'" . implode("','", $subjects) . "'";
    $sel_query12 = "SELECT subcode, subname, examdate, examtime FROM examroutine 
                  WHERE exid='$examid' AND class='$classnumber' AND cgroup='$secgroupname' AND active='1' 
                  AND subcode IN ($subcodes) ORDER BY STR_TO_DATE(examdate, '%Y-%m-%d') ASC";
...
$left_size = ceil($total / 2);
$left_table = array_slice($exam_data, 0, $left_size);
$right_table = array_slice($exam_data, $left_size);
...
<div class="tablepart">
    <div class="tableleft">
        <div class='table-fix'><center><?php renderTable($left_table); ?></center></div>
    </div>
    <div class="tableright">
        <div class='table-fix'><center><?php renderTable($right_table); ?></center></div>
    </div>
</div>
...
<h5 style="color:red;">Scan This QR Code To See Result After Publish</h5>
            ...
            <div class="footerpart_right">
                <center>
                    ...
                    <img src='../img/hmsign.png' style="width:100px;height:30px;margin-top: 40px;">
