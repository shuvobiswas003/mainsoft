...
// Get all sections with their routines
$query = "SELECT sg.id as section_id, sg.classname, sg.secgroupname, 
          cr.day_of_week, cr.period_id, p.period_name, p.start_time, p.end_time,
          t.name as teacher_name
          FROM sectiongroup sg
          LEFT JOIN class_routine cr ON sg.id = cr.section_id
          LEFT JOIN class_periods p ON cr.period_id = p.id
          LEFT JOIN teacher t ON cr.teacher_id = t.id
          ORDER BY sg.classnumber, sg.secgroupname, cr.day_of_week, p.start_time";
$result = mysqli_query($link, $query);

$allRoutines = [];
while ($row = mysqli_fetch_assoc($result)) {
    $sectionId = $row['section_id'];
    if (!isset($allRoutines[$sectionId])) {
        $allRoutines[$sectionId] = [
    ...
    if ($row['day_of_week'] !== null) {
        $allRoutines[$sectionId]['routines'][$row['day_of_week']][$row['period_id']] = [
...
<head>
    ...
    <title>All Class Routines - <?= htmlspecialchars($schoolInfo['schoolname']) ?></title>
...
<body>
        ...
        <div class="print-controls">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h1 class="mb-0" style="color: #1e3c72;">
                    <i class="bi bi-calendar3"></i> All Class Routines
                </h1>
                <div>
            ...
            <div class="alert alert-primary">
                <div class="d-flex flex-wrap justify-content-between">
                    ...
                    <div>
                        <span class="info-badge"><i class="bi bi-collection"></i> Classes</span>
                        <?= count($allRoutines) ?> Sections
                    </div>
                </div>
            </div>
        </div>
    ...
    <?php foreach ($allRoutines as $sectionId => $sectionData): ?>
        <div class="print-page">
            ...
            <h3 class="section-title">
                ...
                <?= htmlspecialchars($sectionData['classname']) ?> - 
                SECTION <?= htmlspecialchars($sectionData['secgroupname']) ?>
            </h3>
                    ...
                    <?php foreach ($dayNames as $dayIndex => $dayName): ?>
                        <tr>
                            ...
                            <?php foreach ($periods as $period): ?>
                                <?php 
                                $periodId = $period['id'];
                                $assignment = $sectionData['routines'][$dayIndex][$periodId] ?? null;
                                $cellClass = $assignment ? 'assigned-cell' : 'unassigned-cell';
                                ?>
                                <td class="<?= $cellClass ?>">
                                    <?php if ($assignment): ?>
            ...
            <div class="signature-container">
                <div class="signature-box">
                    ...
                    <div class="signature-designation"><?= htmlspecialchars($sectionData['classname']) ?></div>
                </div>
                <div class="signature-box">
                   
                    <div class="signature-name"><?= htmlspecialchars($schoolInfo['headname'] ?? 'Headmaster') ?></div>
                    <div class="signature-designation"><?= htmlspecialchars($schoolInfo['head_deg'] ?? 'Headmaster') ?></div>
                </div>
            </div>
